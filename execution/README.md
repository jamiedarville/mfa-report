...
| Script | Does |
| --- | --- |
| `latest-mfa-report.ps1` | Every user who can still sign in with SMS or a phone call, in plain-English columns, flagged Legacy vs Modern. One CSV to `output/`; `-AllUsers` emits the whole directory instead, to filter downstream. Reference implementation of the script contract. |
